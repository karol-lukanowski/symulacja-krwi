...
def update_matrix_upstream(sid:simInputData, vnow, pnow, edges, in_nodes):
    #sprawdzić zmiany ks
    data, row, col = [], [], []
    sresult = np.zeros(2 * sid.nsq)
    diag = np.ones(2 * sid.nsq) * sid.ks
    ...
    return spr.csr_matrix((data, (row, col)), shape=(2 * sid.nsq, 2 * sid.nsq)), sresult
...
def update_graph_upstream(sid:simInputData, snow, pnow, oxresult, edges, update):
    for i,e in enumerate(edges):
        ...
        if oxresult[n1] == 1 and oxresult[n2] == 1:
            if pnow[n1] > pnow[n2]:
                ...
                F = sid.cs * snow[n2]
            update[i] += d_update(F, sid.F_s)
    return update

def update_graph_downstream(sid:simInputData, snow, pnow, oxresult, edges, update):
    for i,e in enumerate(edges):
        n1, n2, d, l, t = e
        if oxresult[n1] == 1 and oxresult[n2] == 1:
            if pnow[n1] < pnow[n2]:
