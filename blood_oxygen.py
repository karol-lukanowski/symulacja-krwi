...
def create_vector_a(sid:simInputData, in_nodes_ox):
    bloodoxresult = np.zeros(sid.nsq)
    for node in in_nodes_ox:
        bloodoxresult[node] = sid.cox_in
    return bloodoxresult

def create_vector_v(sid:simInputData, out_nodes_ox):
    bloodoxresult = np.zeros(sid.nsq)
    for node in out_nodes_ox:
        bloodoxresult[node] = sid.cox_out
    return bloodoxresult

def update_matrix_a(sid:simInputData, pnow, oxresult, bloodoxresult, edges):
    data, row, col = [], [], []
...
def update_matrix_v(sid:simInputData, pnow, oxresult, bloodoxresult, edges):
    data, row, col = [], [], []
    diag = np.ones(sid.nsq) * sid.kb
    for n1, n2, d, l, t in edges:
        if bloodoxresult[n1] == 1:
