...
def set_geometry(n, G=[], geo='rect', R=25, R_s=5, *args, **kwargs):
    ...
    if geo == 'rect':
        ...
        nodes_pos = kwargs['nodes']
        for pos in nodes_pos:
                nodes = De.find_node(G, pos, n)
                in_nodes.append(nodes[0])
                out_nodes.append(nodes[1])
...
def create_edgelist(G, n, in_nodes, out_nodes, reg_nodes, boundary_nodes_out, boundary_nodes_in):
    edges = []
    ...
    for n1, n2 in G.edges():
        ...
        l = G[n1][n2]['length']
        if n2 == n1 + n * n:
            if n1 in in_nodes and n2 in out_nodes:
                edges.append((n1, n2, d, l, -2))
            else:
                edges.append((n1, n2, d, l, -1))
        elif (n1 not in in_nodes and n1 not in out_nodes) and (n2 not in in_nodes and n2 not in out_nodes):
