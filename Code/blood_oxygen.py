...
def create_vector(sid:simInputData, in_nodes_ox):
    bloodoxresult = np.zeros(sid.networkSizeSquared)
...
def update_matrix(sid:simInputData, pnow, oxresult, bloodoxresult, edges):
    data, row, col = [], [], []
    diag = np.ones(sid.networkSizeSquared) * sid.kb
    for n1, n2, d, l, t in edges:
        if bloodoxresult[n1] == 1:
            ...
            if bloodoxresult[n2] == 1:
                ...
                if pnow[n2] < pnow[n1]:
                    res = -(pnow[n1] - pnow[n2]) * d ** 4 * np.pi / (128 * sid.viscosityCoeff * l)
                    data.append(res)
                    row.append(n2)
                    col.append(n1)
                    diag[n2] -= res
            ...
            if oxresult[n1] == 1:
                if pnow[n1] < pnow[n2]:
                    res = -(pnow[n2] - pnow[n1]) * d ** 4 * np.pi / (128 * sid.viscosityCoeff * l)
                    data.append(res)
                    row.append(n1)
                    col.append(n2)
                    diag[n1] -= res
