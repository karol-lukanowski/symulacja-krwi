...
def draw(sid:simInputData, nxGraph, in_nodes, out_nodes, boundary_edges, oxresult, name, data='q'):
    """
    ...
    qmax = max([edge[2] for edge in nxGraph.edges(data=data)])

    edges = []
    qs = []
    colors = []
    for edge in nxGraph.edges(data=data):
            x, y, q = edge
    ...
    nx.draw_networkx_edges(nxGraph, pos, edgelist=edges, width=sid.qdrawconst * np.array(qs) / qmax, edge_color = colors)
    #nx.draw_networkx_nodes(nxGraph, pos, node_size = 30, node_color = oxdraw, cmap='Blues')
...
def drawblood(sid:simInputData, nxGraph, in_nodes, out_nodes, boundary_edges, name, oxresult, oxdraw, data='q'):
    """
    rysowanie krwi, data to q albo d
    """
    plt.figure(figsize=(40, 40), dpi = 200)
    ...
    nx.draw_networkx_edges(nxGraph, pos, edgelist=edges, width=sid.qdrawconst * np.array(qs) / qmax, edge_color='r')
    nx.draw_networkx_nodes(nxGraph, pos, node_size = 30, node_color = oxdraw, cmap='Blues')
...
def draw_initial_d(nxGraph, boundary_edges):
    ds = []
    for edge in nxGraph.edges(data='d'):
            x, y, d = edge
            if (x, y) not in boundary_edges and (y, x) not in boundary_edges:
                ds.append(d)
...
def drawhist(sid:simInputData, nxGraph, in_nodes, out_nodes, boundary_edges, oxresult, pnow, vnow, snow_upstream, snow_downstream, name):
    d_hist = [[], [], [], [], [], [], [], []]
    q_hist = [[], [], [], [], [], [], [], []]
    shear_hist = [[], [], [], [], [], [], [], []]
    vegf_hist = [[], [], [], [], [], [], [], []]
    ...
    q_max = max([edge[2] for edge in nxGraph.edges(data='q')])
    pin = np.max(pnow)

    for n1, n2 in nxGraph.edges():
        q = nxGraph[n1][n2]['q']
        d = nxGraph[n1][n2]['d']
        l = nxGraph[n1][n2]['length']

        shear = sid.F_mult * sid.forceConstC2 * mu_d(d) * q / d ** 3
    ...
    edges = []
    qs = []
    for edge in nxGraph.edges(data="q"):
        x, y, q = edge
        if (x, y) not in boundary_edges and (y, x) not in boundary_edges:
    ...
    plt.scatter(x_in, y_in, s=60, facecolors='white', edgecolors='black')
    plt.scatter(x_out, y_out, s=60, facecolors='black', edgecolors='white')
    nx.draw_networkx_edges(nxGraph, pos, edgelist=edges, edge_color=colors, width=sid.qdrawconst * np.array(qs) / q_max)
    #nx.draw_networkx_nodes(nxGraph, pos, node_size = 25 * oxdraw, node_color = oxdraw, cmap='Reds')
    plt.axis('equal')
    ...
    for hist in d_hist:
        ...
        cindex+=1
    plt.axvline(sid.thresholdDiameter, color='k', linestyle='dashed', linewidth=1)
    plt.yscale("log")
...
def uniform_hist(sid:simInputData, nxGraph, in_nodes, out_nodes, boundary_edges, oxresult, pnow, vnow, snow_upstream, snow_downstream, name):
    d_hist = []
    q_hist = []
    shear_hist = []
    vegf_hist = []
    ...
    for n1, n2 in nxGraph.edges():
        ...
        shear = sid.forceMultiplier * sid.forceConstC2 * mu_d(d) * q / d ** 3
    ...
    plt.scatter(x_in, y_in, s=60, facecolors='white', edgecolors='black')
    plt.scatter(x_out, y_out, s=60, facecolors='black', edgecolors='white')
    nx.draw_networkx_edges(nxGraph, pos, edgelist=edges, edge_color = colors, width=sid.qdrawconst * np.array(qs) / q_max, edge_cmap = plt.cm.plasma)
    #nx.draw_networkx_nodes(nxGraph, pos, node_size = 25 * oxdraw, node_color = oxdraw, cmap='Reds')
    plt.axis('equal')
...
def drawvessels(sid:simInputData, nxGraph, in_nodes, out_nodes, boundary_edges, name, oxresult, oxdraw, data='q'):
    """
    rysowanie krwi, data to q albo d
    """
    plt.figure(figsize=(40, 40), dpi = 200)
    ...
    vessels = np.zeros(sid.networkSizeSquared)

    def find_veins(nxGraph, node, oxresult):
        vessels[node] = 2
        for neigh in nxGraph.neighbors(node):
            if oxresult[neigh] == 1 and vessels[neigh] == 0:
                find_veins(nxGraph, neigh, oxresult)

    def find_arts(nxGraph, node, oxresult):
        vessels[node] = 1
        for neigh in nxGraph.neighbors(node):
            if oxresult[neigh] == 1 and vessels[neigh] == 0:
                find_arts(nxGraph, neigh, oxresult)

    for node in in_nodes:
        find_arts(nxGraph, node, oxresult)
    for node in out_nodes:
        find_veins(nxGraph, node, oxresult)
    ...
    nx.draw_networkx_edges(nxGraph, pos, edgelist=edges, width=sid.qdrawconst * np.array(qs) / qmax, edge_color=colors)
    nx.draw_networkx_nodes(nxGraph, pos, node_size = 15, node_color = oxdraw, cmap='Blues', vmax = 5e6)
