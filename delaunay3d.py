...
def Build_delaunay_net(n, periodic = 'top', noise = ["uniform", 1, 1], h = 5):
    ...
    for edge, l in zip(edges, edges_lengths):
        ...
        if (n1 < nkw) and (n2 < nkw):
            ...
            final_edges.append((n1 + nkw, n2 + nkw))
            final_edges_lengths.append(l)
        elif (n1 < nkw) and (n2 >= nkw) and (n2 < 2*nkw):
            final_edges.append((n1, n2-nkw))
            boundary_edges.append((n1, n2-nkw))
            final_edges_lengths.append(l)
            final_edges.append((n1 + nkw, n2))
            boundary_edges.append((n1 + nkw, n2))
            final_edges_lengths.append(l)
        elif (n1 < nkw) and (n2 >= 3 * nkw) and (n2 < 4*nkw):
            final_edges.append((n1, n2-3 * nkw))
            boundary_edges.append((n1, n2-3 * nkw))
            final_edges_lengths.append(l)
            final_edges.append((n1 + nkw, n2 - 2 * nkw))
            boundary_edges.append((n1 + nkw, n2 - 2 * nkw))
            final_edges_lengths.append(l)

    for i in range(nkw):
        final_edges.append((i, i + nkw))
        final_edges_lengths.append(h)


    G = nx.Graph()
    G.add_nodes_from(list(range(2 * nkw)))
    ...
    for node in G.nodes:
        if node < nkw:
            ...
            G.nodes[node]["pos"] = points[node - nkw]
