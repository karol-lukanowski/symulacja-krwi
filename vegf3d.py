...
def update_matrix(sid:simInputData, vresult, edges):
    ...
    return spr.csr_matrix((data, (row, col)), shape=(2 * sid.nsq, 2 * sid.nsq))
    

def update_graph(sid:simInputData, vnow, oxresult, edges, update):
    for i,e in enumerate(edges):
        n1, n2, d, l, t = e
        if oxresult[n1] == 1 or oxresult[n2] == 1:
            F = sid.F_mult_ox * np.abs(vnow[n1] - vnow[n2])/l
            update[i] += d_update(F, sid.F_ox)
    return update


def update_blood(sid:simInputData, oxresult, edges, update):
    ...
    for i,e in enumerate(edges):
        ...
        if d > sid.dmax:
            ...
            d = sid.dmin
        if oxresult[n1] == 1 and oxresult[n2] == 1 and n2 == n1 + sid.nsq:
            d = 0.001
        edges[i] = (n1, n2, d, l, t)
    return oxresult, edges